...
dnf install -y acpid binutils coreutils dbus-devel dbus-glib-devel dbus-libs dkms dnf-plugins-core emacs exa fish fontawesome-fonts glib2-devel gnome-tweaks hanazono-fonts htop libtool kernel-devel kernel-devel-matched kernel-headers linux-firmware make NetworkManager-tui p7zip pciutils powerline-fonts snapd toolbox util-linux-user vulkan-headers vulkan-tools
