...
apt install -y acpid awscli binutils cargo celluloid clojure curl dkms elixir fd-find ffmpeg fish fonts-firacode fonts-ibm-plex gcc golang htop linux-firmware make mesa-vulkan-drivers make npm nvidia-driver-510 openjdk-18-jdk p7zip pciutils pip powerline qt5-style-plugins ripgrep rustc shotwell ubuntu-restricted-extras vulkan-tools
